...
- **Deferred (per spec "Out of scope"):** LLM-drafted free-text answers, resume/file auto-attach + Blob storage, fully custom/canvas widgets beyond the screenshot fallback, Gmail (Slice 3), OAuth LLM backend.
- **Built but currently INERT (wired in a later slice):**
  - **`atsPatterns` reuse-on-plan.** The store + repo (`getPattern`/`mergePattern`) exist and are tested, but the planner is not yet passed remembered mappings and nothing calls `mergePattern` after a run. The "faster/cheaper on repeat sites" behavior does not exist yet — it is a follow-up.
  - **Screenshot re-plan pass.** The LLM client accepts images and the planner accepts an `images` arg, but the service worker does not yet capture/crop a screenshot for `needsVisual` fields. Instead, `partitionPlan` now routes any `needsVisual: true` field to a **checkpoint** (the user fills it), so the hybrid path degrades safely to manual input rather than guessing. Wiring `chrome.tabs.captureVisibleTab` + crop + re-plan is a follow-up.
- **Post-review fix (commit after Task 14):** `PLAN_PARTITIONED` now always transitions to `filling` (not directly to `checkpoint`) so confident fills are applied even on a screen that also has checkpoints; `needsVisual` fields are gated to checkpoints; the checkbox executor path gained a test.
